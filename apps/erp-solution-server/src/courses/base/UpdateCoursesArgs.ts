/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CoursesWhereUniqueInput } from "./CoursesWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { CoursesUpdateInput } from "./CoursesUpdateInput";

@ArgsType()
class UpdateCoursesArgs {
  @ApiProperty({
    required: true,
    type: () => CoursesWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => CoursesWhereUniqueInput)
  @Field(() => CoursesWhereUniqueInput, { nullable: false })
  where!: CoursesWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => CoursesUpdateInput,
  })
  @ValidateNested()
  @Type(() => CoursesUpdateInput)
  @Field(() => CoursesUpdateInput, { nullable: false })
  data!: CoursesUpdateInput;
}

export { UpdateCoursesArgs as UpdateCoursesArgs };
